using RimWorld;
using Verse;
using Verse.AI;

//TODO: Fix this class
namespace rjw
{
	public class ThinkNode_ConditionalCanRapeCP : ThinkNode_Conditional
	{
		protected override bool Satisfied(Pawn pawn)
		{
			if (!xxx.config.comfort_prisoners_enabled)
			{
				return false;
			}
			if (pawn == null || pawn.Faction == null)
			{
				Logger.Message("[RJW]ThinkNode_ConditionalCanRapeCP::satisfied called 0");
				return false;
			}
			// Due to the existence of whore system, no longer allow pawns from other factions to rape comfort prisoners
			if (Mod_Settings.WildMode) return true;
			if (!pawn.Faction.IsPlayer)
			{
				Logger.Message("[RJW]ThinkNode_ConditionalCanRapeCP::satisfied called 1");
				return false;
			}
			if (pawn.Map == null || pawn.Map != Find.VisibleMap)
			{
				Logger.Message("[RJW]ThinkNode_ConditionalCanRapeCP::satisfied called 2");
				return false;
			}
			if (pawn.IsPrisonerOfColony)
			{
				Logger.Message("[RJW]ThinkNode_ConditionalCanRapeCP::satisfied called 3");
				return false;
			}
			if (xxx.config.animals_enabled && xxx.is_animal(pawn))
			{
				Logger.Message("[RJW]ThinkNode_ConditionalCanRapeCP::satisfied called 4");
				return true;
			}
			else if (xxx.is_human(pawn))
			{
				return xxx.isSingleOrPartnerNotHere(pawn);
			}
			else return false;
		}
	}
}