...
namespace rjw
{
	...
	internal static class PATCH_Hediff_Pregnant_DoBirthSpawn
	{
		...
		private static bool on_begin_DoBirthSpawn(ref Pawn mother, ref Pawn father)
		{
			...
			Logger.Message("patches_pregnancy::PATCH_Hediff_Pregnant::DoBirthSpawn() called");
			...
			//Logger.Message("Hediff_Pregnancy::DoBirthSpawn( " + mother_name + ", " + father_name + ", " + chance_successful + " ) - generating baby pawns");
			...
			//Logger.Message("Hediff_Pregnancy::DoBirthSpawn( " + mother_name + ", " + father_name + ", " + chance_successful + " ) - creating spawn request");
			...
			//Logger.Message("Hediff_Pregnancy::DoBirthSpawn( " + mother_name + ", " + father_name + ", " + chance_successful + " ) - removing pregnancy");
    ...
    class PATCH_Hediff_Pregnant_Tick {
        ...
        static bool on_begin_Tick( Hediff_Pregnant __instance ) {
            ...
            //    Logger.Message("patches_pregnancy::PATCH_Hediff_Pregnant::Tick( " + __instance.pawn.NameStringShort + " ) - gestation_progress = " + __instance.GestationProgress);
    ...
    */
    ...
    class PATCH_PawnRenderer_RenderPawnInternal {
        ...
        static bool on_begin_RenderPawnInternal(PawnRenderer __instance, Vector3 rootLoc, Quaternion quat, bool renderBody, Rot4 bodyFacing, Rot4 headFacing, RotDrawMode bodyDrawType = RotDrawMode.Fresh, bool portrait = false, bool headStump = false) {
            Logger.Message("PATCH_PawnRenderer_RenderPawnInternal() called");
    ...
    class PATCH_PawnGraphicSet_ResolveAllGraphics {
        ...
        static bool on_begin_ResolveAllGraphics(PawnGraphicSet __instance) {
            Logger.Message("PATCH_PawnGraphicSet_ResolveAllGraphics::ResolveAllGraphics() called");
            if (__instance.pawn.RaceProps.Humanlike && __instance.pawn.ageTracker.CurLifeStageIndex < 4) {
                Logger.Message("   " + __instance.pawn.NameStringShort + ":  humanlike = true, lifeStage = " + __instance.pawn.ageTracker.CurLifeStageIndex);
                if (__instance.nakedGraphic != null) {
                    if (__instance.nakedGraphic.drawSize != null) {
                        __instance.nakedGraphic.drawSize *= 0.5f;
                    } else {
                        Logger.Message("   __instance.nakedGraphic.drawSize is null");
                    }
                } else {
                    Logger.Message("   __instance.nakedGraphic is null");
                }
                if (__instance.apparelGraphics != null) {
                    Logger.Message("   __instance.apparelGraphic is present");
                } else {
                    Logger.Message("   __instance.apparelGraphic is null");
                }
                if (__instance.rottingGraphic != null) {
                    Logger.Message("   __instance.rottingGraphic is present");
                } else {
                    Logger.Message("   __instance.rottingGraphic is null");
                }
            }

            if (__instance.pawn.RaceProps.Humanlike && __instance.pawn.ageTracker.CurLifeStageIndex < 4) {
                Logger.Message("PATCH_PawnGraphicSet_ResolveAllGraphics::ResolveAllGraphics() - adjusting draw size for " + __instance.pawn.NameStringShort + "");
                if (__instance != null) {
                    var x = __instance.nakedGraphic.drawSize.x;
                    var y = __instance.nakedGraphic.drawSize.y;
                    Logger.Message("   current size = " + x + "/" + y + ", new size = " + x * 0.5f + "/" + y * 0.5f);
                } else {
                    Logger.Message("   __instance == null");
                }
            }
