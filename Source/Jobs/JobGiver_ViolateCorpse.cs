...
namespace rjw
{
	public class JobGiver_ViolateCorpse : ThinkNode_JobGiver
	{
		...
		{
			Logger.Message("JobGiver_ViolateCorpse::find_corpse( " + rapist.NameStringShort + " ) called");
			Corpse found = null;
			...
			{
				Logger.Message(rapist.NameStringShort + " found a corpse with id " + corpse.Label);
				if (rapist.CanReserve(corpse, 1, 0) && !corpse.IsForbidden(rapist))
				{
					...
					Logger.Message("   " + corpse.InnerPawn.NameStringShort + " =  " + freshness + "/" + distance + ",  best =  " + best_freshness + "/" + best_distance);
		...
		{
			Logger.Message("[RJW] JobGiver_ViolateCorpse::TryGiveJob( " + p.NameStringShort + " ) called");
			if ((Find.TickManager.TicksGame >= p.mindState.canLovinTick) && (p.CurJob == null))
			{
				if (xxx.is_healthy(p) && xxx.can_rape(p) && !comfort_prisoners.is_designated(p))
				{
					...
					Logger.Message("[RJW] JobGiver_ViolateCorpse::TryGiveJob - target is " + (target == null ? "NULL" : "Found"));
					if (target != null)
					{
