...
namespace rjw
{
	internal class JobDriver_RapeEnemyByInsect : JobDriver_RapeEnemy
	{
		...
		protected override void Impregnate(Pawn pawn, Pawn part, bool isAnalSex)
		{
			if (xxx.is_human(part))
			{
				if (pawn.gender == Gender.Female)
				{
					...
					if (egg != null)
					{
						Logger.Message("[RJW]JobDriver_RapeEnemyByInsect::aftersex() - Planting egg " + egg.ToString());
						PlantSomething(egg, part, isAnalSex, Rand.Range(1, 2));
					}
					/*else
					{
						Logger.Message("[RJW]JobDriver_RapeEnemyByInsect::aftersex() - There is no EggData of " + pawn.def.defName);
					}*/
				}
				else
				{
					Logger.Message("[RJW]JobDriver_RapeEnemyByInsect::aftersex() - Fertilize eggs");
		...
		public override float GetFuckability(Pawn rapist, Pawn target)
		{
			...
			{
				Logger.Message("[RJW]" + this.GetType().ToString() + "::GetFuckability(" + rapist.ToString() + ") - going to plant egg ->"+ target.ToString());
